...
template<typename T>
inline void
QuadDomain<T>::configure_d(int qdeg,
        bool poswts,
...
QuadDomain<T>::nbfn_for_qdeg(int qdeg)
{
    int n = 0;
...
template<typename T>
inline void
QuadDomain<T>::clamp_arg(int i, int aoff, VectorXT& args)
{
    switch (i)
    {
        case 0:
...
template<typename T>
inline void
QuadDomain<T>::sort_arg(int i, int aoff, VectorXT& args)
{
