function [SBPError, CSVError] = getSBP_Numeric(xOutr, xShap, xSoln, xFlux, nFluF, shapBasis, compBasis)
...
%       shapBasis -- a function handle which returns the evaluated basis to be used for geometrically fitting the shape at coordinates, as a function of x and d [= @(x,d) x(1) + x(2);]
%       compBasis -- a function handle which returns the evaluated basis to be used for determining the solution computationally, as a function of x and d [= @(x,d) x(1) + x(2);]
...
[xIntegrate, wIntegrate] = getIntegrationWeights(compPoly);
...
% -- Get the mass matrix for the quadrature
Mq = diag(wIntegrate);
...
for iFace = 1:nFace
    ...
    sTC(:,sum(nFluF(1:iFace-1))+1:sum(nFluF(1:iFace))) = ((lT .* w1DInt') * lT');
...
% -- HELPER FUNCTIONS
function [xG, wG] = getIntegrationWeights(pGon)
    ...
    for iT = 1:size(T,1)
        ...
        Q(iT) = quadtriangle(500, 'Domain', xTri,'Type','product');
    end
    xG = vertcat(Q.Points); wG = vertcat(Q.Weights);
