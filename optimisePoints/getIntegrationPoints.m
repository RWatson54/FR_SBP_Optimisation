function [xInt, wInt] = getIntegrationPoints(compPoly, nInt)

%GETPOLYSYMMETRY generates the integeration points and weights over a polygon structure

% This code is essentially a wrapper for the FEX quadtriangle, which is...substantial.

% -------------------------------------------------------------

% -- The inputs are:

%       compPoly -- The polygon structure to be integrated
%       nInt -- the desired degree of the integration (not the number of points)

% -- The outputs are:

%       xInt -- the integration point coordinates [nIntg -x- nDim]
%       wInt -- the integration point weights [nIntg -x- 1]

% -------------------------------------------------------------

% -- Triangulate the polygon shape structure
T = triangulation(compPoly);

% -- And loop over all the triangles, building the list
for iT = 1:size(T,1)
    xTri =  T.Points(T.ConnectivityList(iT,:),:);
    %Q(iT) = quadtriangle(25, 'Domain', xTri,'Type','nonproduct');
    Q(iT) = quadtriangle(nInt, 'Domain', xTri,'Type','product');
end

% -- Concatenate the points and weights of each subtriangle into the whole domain
xInt = vertcat(Q.Points); wInt = vertcat(Q.Weights);
...
%  [R1] J. Radon, "Zur mechanischen Kubatur," Monatshefte f�r Mathematik,
%       52,286--300, 1948.
